...
test('16 player gamestate losses', async () => {
    ...
    expect(round1results.players["personaId4"].rank).toBeGreaterThanOrEqual(8)

    var round2results = await SimulateRound(round1results, "personaId4", "loss")
    expect(round2results.players["personaId4"].rank).toBeGreaterThanOrEqual(12)

    var round3results = await SimulateRound(round2results, "personaId4", "loss")
    expect(round3results.players["personaId4"].rank).toBeGreaterThanOrEqual(15)

    var round4results = await SimulateRound(round3results, "personaId4", "loss")
    expect(round4results.players["personaId4"].rank).toBeGreaterThanOrEqual(16)
